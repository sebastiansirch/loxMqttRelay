...
reproducible reason (Bug 1). While building the test harness, a second,
independent issue was also discovered (Bug 2), which can likewise cause
silent message loss.
...
## Bug 2: `base_topic` overlap causes silent message loss
...
loss occurred exclusively on the HTTP forwarding path to the Miniserver, consistent with Bug 1.
...
- Found and fixed a bug: `send_to_miniserver_via_http` opened a new `aiohttp.ClientSession` (i.e.
  ...
  handler instance.
- Found and fixed a second bug: messages on subscriptions that fall inside the `base_topic`
...
- `tests/test_http_miniserver_handler.py`, `tests/test_mqtt_relay.py`: tests for both fixes.
- `loadtest/`: new, standalone Docker Compose load test setup (not part of the regular test
  suite/CI, run manually).

### Test Plan
- [x] `pytest` (204 existing + 6 new tests) passes, run inside the Docker build image (the
      project requires Python 3.14 + the compiled Rust extension).
...
- [x] Load test after the fix: 50,000 messages, twice in a row with no restart in between
      (150,000 total), 0% loss across all three scenarios; no errors in the relay log.
- [x] `udp_to_mqtt` scenario as a control: no loss at any point (confirms the problem was
      specific to the HTTP forwarding path).
